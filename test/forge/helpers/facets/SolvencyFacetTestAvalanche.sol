...
import {SolvencyFacetProdAvalanche} from "../../../../contracts/facets/avalanche/SolvencyFacetProdAvalanche.sol";
import {TestAuthorisedSigners} from "../TestAuthorisedSigners.sol";

/**
 * @title SolvencyFacetTestAvalanche
 * @notice Production solvency facet with ONLY the signer set swapped to the 5 DP test
 *         signers (keccak256(abi.encodePacked("DP_RS_SIGNER", uint256(i)))).
 *
 ...
 * `vm.etch(target, type(SolvencyFacetTestAvalanche).runtimeCode)` in tests,
 * NOT via `new SolvencyFacetTestAvalanche()`. Forge does NOT enforce EIP-170
 * on vm.etch, so the etch succeeds even on the 24,538B+ bytecode.
 *
 * Both SolvencyFacetProdAvalanche (via PrimaryProdDataServiceConsumerBase →
 ...
 */
contract SolvencyFacetTestAvalanche is SolvencyFacetProdAvalanche, TestAuthorisedSigners {
    /**
