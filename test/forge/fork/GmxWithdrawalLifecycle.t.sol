...
/**
 * SP5 Task 5 — GMX WITHDRAWAL lifecycle (Arbitrum live-diamond fork). Gated — SKIPS under the
 * default test config (RUN_GMX_FORK=true + arbitrum chain config required).
 *
 * Builds on the proven deposit+execute flow (T4): a simulated keeper first executes a GM
 * deposit so the Prime Account holds a REAL GM position, then the account creates a GM
 ...
 * UNFREEZE + exposure sync; we assert BOTH of those AND the token return (GM burned, WETH+USDC
 * up) — together they distinguish a real execution from both a swallowed callback and a cancel.
 */
contract GmxWithdrawalLifecycleTest is ArbitrumGmxForkFixture {
    function testWithdrawalExecutedByKeeperFiresCallback() public {
        ...
        // 2) create a FULL GM withdrawal. minLong/minShort kept tiny (~$2 + $1, well below the
        //    ~$200+ the position returns) so the keeper execution fills both legs; the fixture
        //    sizes the RedStone GM price to the min-output USD so create-side isWithinBounds
        //    passes at its centre.
        uint256 minLong = 0.001 ether; // ~$2 of WETH at the deterministic $2000 test price
        ...
        // 3) simulate the keeper executing the withdrawal in a separate "tx".
        ...
        GmxKeeperSim.executeWithdrawal(vm, key, tokens, providers);
        ...
        // processed the withdrawal (WETH also picks up the wrapped execution-fee refund).
        assertGt(wethAfter, wethBefore, "WETH (long) not returned to the loan");
        ...
        assertTrue(_ownsAsset(SYM_ETH), "ETH (long) not synced into owned assets");
